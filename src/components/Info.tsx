...
import { useSwapContext, useSwapFair ,useFtrFair} from "../context/Swap";
...
export function InfoLabel() {
  ...
  const toMintInfo = useMint(fromMint);
  let fair = useSwapFair();
  if (fair!=undefined){
  if (fair>0){
    fair=1/fair;
  }}
  const fair_ftr = useFtrFair();

  const tokenMap = useTokenMap();
  let fromTokenInfo = tokenMap.get(fromMint.toString());
  let toTokenInfo = tokenMap.get(toMint.toString());

  let msg_to_return="You're BUYING Quarterly Fixed Rate at the rate : ";
  if (fair!=undefined){
  if (fair>1){
    msg_to_return="You're REDEEMING Quarterly Fixed Rate at the rate : ";
  }}
  return (
